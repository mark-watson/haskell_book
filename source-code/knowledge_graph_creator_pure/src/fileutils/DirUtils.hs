-- Directory iteration helpers for simple batch processing
module DirUtils
  ( iterateOverDir
...
-- Apply `fn` to each file in `dir_path` (skips `.` and `..`)
...
-- Apply `fn` to each file whose name matches `filterFn`, passing full paths
