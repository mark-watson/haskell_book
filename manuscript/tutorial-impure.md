...
So far we have been using **data** to define new types and **newtype** is similar except **newtype** acts during compile time and no type information is present at runtime. All monads conatin a value and for the State monad this value is a function. The **>>=** operator is called the *bind* operator.
...
I don't usually like using the **trace** functions because debugging with them involves slightly rewriting my code. My preference is to get low level code written interactively in the GHCI repl so it does not need to be deugged. I very frequently use print statement inside **IO**s since adding them requires no significant modification of my code.
